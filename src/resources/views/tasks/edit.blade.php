    ...
    <div class="container">
        ...
        <form action="{{ route('tasks.update', $task->id) }}" method="POST" novalidate>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="progress" class="form-control" step="1" min="0" max="100" value="{{ old('progress', $task->progress ?? 0) }}" placeholder="進捗 (0〜100)%">
                @error('progress')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="parent_id" class="form-select">
                    ...
                    @foreach($tasks as $task)
                        <option value="{{ $task->id }}" {{ old('parent_id', $task->parent_id) == $task->id ? 'selected' : '' }}>
                            {{ $task->title }}
                        </option>
                    @endforeach
                </select>
